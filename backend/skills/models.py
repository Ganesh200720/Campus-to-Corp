...
class AssessmentQuestion(models.Model):
    ...
    # Optional so existing questions continue to work.
...
class StudentTopicProgress(models.Model):
    """
    Tracks a student's progress for an individual topic assessment.
    """
    ...
    def __str__(self):
        return (
            ...
            f"{self.topic.title}: {self.score}%"
        )


class StudentModuleProgress(models.Model):
    """
    Tracks a student's module-level assessment.

    A module assessment should cover the topics inside that module.
    """
    ...
    def __str__(self):
        return (
            f"{self.student.username} - "
            f"{self.module.title}: {self.score}%"
        )


# ---------------------------------------------------------------------------
# Industry-created assessments
# ---------------------------------------------------------------------------
...
class IndustryAssessmentAttempt(models.Model):
    ...
    student = models.ForeignKey(
        User,
        on_delete=models.CASCADE,
        related_name='industry_assessment_attempts'
    )
    ...
    def __str__(self):
        return (
            f"{self.student.username} -> "
            f"{self.assessment.title}: {self.percentage}%"
        )
