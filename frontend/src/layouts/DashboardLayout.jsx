...
import {
  ...
  PlusCircle, GraduationCap, Presentation, BriefcaseBusiness,
} from 'lucide-react'
...
const NAV = {
  ...
  industry: [
    ...
    { to: '/industry/post-collaboration', label: 'Post Collaboration', icon: Presentation },
    { to: '/industry/my-collaborations', label: 'My Collaborations', icon: BriefcaseBusiness },
    { to: '/industry/internships', label: 'My Internships', icon: BriefcaseBusiness },
    { to: '/industry/jobs', label: 'My Jobs', icon: BriefcaseBusiness },
    { to: '/industry/applications', label: 'Applications', icon: FileText },
    { to: '/industry/candidate-matching', label: 'Candidate Matching', icon: Users },
    { to: '/industry/collaboration-requests', label: 'Collaboration Requests', icon: ListChecks },
  ],
  faculty: [
    { to: '/faculy', label: 'Dashboard', icon: LayoutDashboard, end: true },
    { to: '/faculty/opportunities', label: 'Opportunities', icon: Presentation },
    { to: '/faculty/collaborations', label: 'Industry Collaborations', icon: Users },
    { to: '/faculty/my-collaborations', label: 'My Requests', icon: ListChecks },
  ],
  admin: [
    { to: '/admin', label: 'Institution Analytics', icon: BarChart3, end: true },
    { to: '/admin/collaborations', label: 'Industry Collaborations', icon: Presentation },
    { to: '/admin/my-collaborations', label: 'My Requests', icon: ListChecks },
  ],
}
...
export default function DashboardLayout({ children }) {
  ...
  const initial = (displayName || user.username || '?')
    .charAt(0)
    .toUpperCase()
  ...
  return (
    <div className="min-h-screen flex bg-slate-50">
      <aside
        className="w-64 h-screen sticky top-0 bg-white border-r border-slate-200
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header
          className="h-16 bg-white border-b border-slate-200 flex items-center
                     justify-between px-6 shrink-0 sticky top-0 z-10"
        >
          <div>
