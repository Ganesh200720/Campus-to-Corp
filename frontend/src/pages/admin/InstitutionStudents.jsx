...
import { Users, Briefcase, Award, Github, Linkedin, ExternalLink } from 'lucide-react'
...
export default function InstitutionStudents() {
  ...
  if (loading) return <LoadingState label="Loading student profiles..." />
    ...
    <div className="space-y-6">
      <div className="card p-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-800">Institution Students</h1>
          ...
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-7">
            ...
            <div className="grid md:grid-cols-2 gap-4 mt-6">
              <div>
                ...
                <p className="font-medium">{selected.degree}</p>
              </div>
              ...
              <div>
                ...
                <p className="font-medium">{selected.placement_readiness}%</p>
              </div>

              <div>
                <p className="text-xs text-slate-400">Profile Completion</p>
                <p className="font-medium">{selected.profile_completion}%</p>
              </div>
            </div>
            ...
            {selected.bio && (
              <div className="mt-6">
                ...
                <p className="text-sm text-slate-600">{selected.bio}</p>
              </div>
            )}
            ...
            <div className="grid grid-cols-3 gap-3 mt-6">
              <div className="card p-4 text-center">
                <Briefcase size={18} className="mx-auto mb-2 text-amber-500" />
                <p className="font-bold">{selected.internship_count}</p>
                <p className="text-xs text-slate-500">Internships</p>
              </div>

              <div className="card p-4 text-center">
                <Award size={18} className="mx-auto mb-2 text-emerald-500" />
                <p className="font-bold">{selected.placement_count}</p>
                <p className="text-xs text-slate-500">Placements</p>
              </div>

              <div className="card p-4 text-center">
                <Users size={18} className="mx-auto mb-2 text-brand-500" />
                <p className="font-bold">{selected.placement_readiness}%</p>
